...
def collision_check(x0, y0, x1, y1, ground_truth, robot_belief):
    ...
    while 0 <= x < ground_truth.shape[1] and 0 <= y < ground_truth.shape[0]:
        ...
        if k == 1 and collision_flag < max_collision: # occupied in GT (==1)
            collision_flag += 1
            if collision_flag >= max_collision:
                break

        if k != 1 and collision_flag > 0: # IF COLLIDED AND GO PAST COLLISION STOP UPDATE
            break

        if x == x1 and y == y1: # END IF REACH FINAL POINT
            break

        robot_belief.itemset((y, x), k)
        # HERE UPDATES COORDS WITH K TLDR IT CHECKS IN A LINE THAT IS WITHIN MAP

        # USING THE B SMT LINE ALGO TO CHECK STR LINE IN DIRECTION
        if error > 0:
            x += x_inc
            error -= dy
        else:
