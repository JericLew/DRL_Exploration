...
class Runner(object): # Run Local
    def __init__(self, meta_agent_id):
        self.meta_agent_id = meta_agent_id
        self.device = torch.device('cuda') if USE_GPU else torch.device('cpu')
        self.local_network = Global_Policy(INPUT_DIM, hidden_size=HIDDEN_SIZE)
    ...
    def do_job(self, episode_number):
        ...
        # save_img = True
        worker = Worker(self.meta_agent_id, self.local_network, episode_number, device=self.device, save_image=save_img, greedy=False)
